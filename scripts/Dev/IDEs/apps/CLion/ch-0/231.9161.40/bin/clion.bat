...
:: Try (in order): CLION_JDK, clion64.exe.jdk, ..\jbr, JDK_HOME, JAVA_HOME.
:: ---------------------------------------------------------------------
SET JRE=

IF NOT "%CLION_JDK%" == "" (
  IF EXIST "%CLION_JDK%" SET "JRE=%CLION_JDK%"
)

SET _JRE_CANDIDATE=
IF "%JRE%" == "" IF EXIST "%APPDATA%\JetBrains\CLion2023.1\clion64.exe.jdk" (
  SET /P _JRE_CANDIDATE=<"%APPDATA%\JetBrains\CLion2023.1\clion64.exe.jdk"
)
IF "%JRE%" == "" (
...
IF NOT EXIST "%JAVA_EXE%" (
  ...
  ECHO No JRE found. Please make sure CLION_JDK, JDK_HOME, or JAVA_HOME point to a valid JRE installation.
  EXIT /B
)
...
:: ---------------------------------------------------------------------
IF NOT "%CLION_PROPERTIES%" == "" SET IDE_PROPERTIES_PROPERTY="-Didea.properties.file=%CLION_PROPERTIES%"

SET VM_OPTIONS_FILE=
SET USER_VM_OPTIONS_FILE=
IF NOT "%CLION_VM_OPTIONS%" == "" (
  :: 1. %<IDE_NAME>_VM_OPTIONS%
  IF EXIST "%CLION_VM_OPTIONS%" SET "VM_OPTIONS_FILE=%CLION_VM_OPTIONS%"
)
IF "%VM_OPTIONS_FILE%" == "" (
  :: 2. <IDE_HOME>\bin\[win\]<exe_name>.vmoptions ...
  IF EXIST "%IDE_BIN_DIR%\clion64.exe.vmoptions" (
    SET "VM_OPTIONS_FILE=%IDE_BIN_DIR%\clion64.exe.vmoptions"
  ) ELSE IF EXIST "%IDE_BIN_DIR%\win\clion64.exe.vmoptions" (
    SET "VM_OPTIONS_FILE=%IDE_BIN_DIR%\win\clion64.exe.vmoptions"
  )
  ...
  ) ELSE IF EXIST "%APPDATA%\JetBrains\CLion2023.1\clion64.exe.vmoptions" (
    SET "USER_VM_OPTIONS_FILE=%APPDATA%\JetBrains\CLion2023.1\clion64.exe.vmoptions"
  )
)
...
SET "CLASS_PATH=%IDE_HOME%\lib\app.jar"
...
SET "CLASS_PATH=%CLASS_PATH%;%IDE_HOME%\lib\intellij-coverage-agent-1.0.706.jar"
...
SET "CLASS_PATH=%CLASS_PATH%;%IDE_HOME%\lib\junit5.jar"
